...
String dataDir = 'data/smallAlcohol'; //'data/alcohol_test'; //'data/smallAlcohol';
...
def mdp;
if (version==1)
	mdp = new dqnMDP_NN(X, Y, Z, negFeatPreds, model, data, wlTruthDB, trainPart, 
		config, generalPredArgsMap, null, friendPreds, networkPreds);
if (version == 2)
	mdp = new dqnMDP_NN2(X, Y, Z, negFeatPreds, model, data, wlTruthDB, trainPart, 
		config, generalPredArgsMap, null, friendPreds, networkPreds);
if (version == 3)
	mdp = new dqnMDP_hindsight(X, Y, Z, negFeatPreds, model, data, wlTruthDB, trainPart, 
		config, generalPredArgsMap, null, friendPreds, networkPreds);

/*
 * Training Start
 */
def outputPath = config.getString('experiment.output.outputdir', 'result/DQN/generator'+ version+ '/');
double[] trend = mdp.training();

println("Finish training")

double accumulatedReward = mdp.optimalSolution();
...
mdp.saveDQN(outputPath+ 'dqn.ser');
...
ps = new PrintStream(new FileOutputStream(outputPath+ "convergence.txt", false));
System.setOut(ps);
 ...
 */
//dqnFileName = 'result/DQN/generator'+version+'/dqn.ser';
//mdp.loadDQN(dqnFileName);
//accumulatedReward = mdp.optimalSolution();
//
//Map<String, Double> predicationMap = new HashMap<String, Double>();
...
//}
//
//ps = new PrintStream(new FileOutputStream('result/DQN/generator'+version+'/predictedRecover.txt', false));
//System.setOut(ps);
//BufferedReader reader = new BufferedReader(new FileReader(dataDir+ '/recovers.txt'));
//while ((read = reader.readLine()) != null) {
//	String[] splited = read.split(",");
//	String user = splited[0];
//	String truth = splited[1];
//	double predicted = predicationMap.get(user);
//	System.out.println(""+user+ ","+truth+","+predicted);
//}
//reader.close();
