package binghamton.rl.DQN.hindsight;
...
//	int maxStep;
	...
	public dqnMDP_hindsight(StandardPredicate[] X, StandardPredicate[] Y, StandardPredicate[] Z, StandardPredicate[] negFeatPreds,
...
//		System.out.println("Area under ROC curve: "+ accumulatedReward);
	...
	public double[] training() throws ClassNotFoundException, IllegalAccessException, InstantiationException {
		...
		int bestEpochIdx = 0;
		List<StateActTrans> HER = new ArrayList<StateActTrans>();
...
////						}
////						if (value >= bestScore && bestScore != 0) {
////							System.out.println("*******************");
////							System.out.println(""+ t+ " ["+ value + ": ["+ valueBefore+ ", "+ valueHat+"]");
////						}
//					}
				...
				if (END_SIGNAL) {
					if (accumulatedReward > bestScore) {
						...
						bestEpochIdx = epoch;
						System.out.println("\nUpdate Best Score: "+ bestScore+ ", "+ optimalRuleList);
						
						// Update Hindsight Experiment Replay
						HER = expReplay.getLastTrajectory();
					} 
					else {
						for (int r=HER.size()-1; r>=0; r--) {
//							if (r==0)
//								System.out.println("HER reward"+ HER.get(r).reward);
							expReplay.store(HER.get(r));
						}
					}
				}
			}
...
//					System.out.println("Best Score: "+ bestScore+ " at Epoch "+bestEpochIdx);
					System.out.println("Optimal Policy Solution: "+ optimalSolution());
...
//		for (int i=0; i<500; i++) {
////			ArrayList<StateActTrans> transBatch = expReplay.getBatch();
//			Pair<INDArray, INDArray> target_data = setTarget((ArrayList<StateActTrans>) HER);
//			
//			INDArray output = currentDQN.output(target_data.getFirst());
//			
//			currentDQN.fit(target_data.getFirst(), target_data.getSecond());
//			
//			INDArray afterOutput = currentDQN.output(target_data.getFirst());
//			for (int t=HER.size()-1; t>=0; t--) {
//				StateActTrans trans = HER.get(t);
//				int p = trans.action;
//				double value = target_data.getSecond().getDouble(t,p);
//				double valueBefore = output.getDouble(t,p);
//				double valueHat = afterOutput.getDouble(t,p);
//				System.out.println(""+ (HER.size()-t)+ " ["+ value + ": ["+ valueBefore+ ", "+ valueHat+"]");
//			}
//			if (i % 50 == 0) {
//				updateTargetNetwork();
...
//		}
	...
	public double AUCcost() throws ClassNotFoundException, IllegalAccessException, InstantiationException {
		...
		double reward = SCALING* reward_auc; //+ reward_grounded;
		if (loss_grounded == 0) 
			reward = 0;
		return reward;
