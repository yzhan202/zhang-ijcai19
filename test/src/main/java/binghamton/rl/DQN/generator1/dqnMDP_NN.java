...
//	int maxStep;
	...
	public dqnMDP_NN(StandardPredicate[] X, StandardPredicate[] Y, StandardPredicate[] Z, StandardPredicate[] negFeatPreds,
		...
		this.negPredNum = allNegPreds.length; //negFeatPreds.length+ 1;
	...
	void populateDatabase(DataStore data, Database dbToPopulate, Partition populatePartition, Set inferredPredicates){
		...
		for (int i=0; i<Y.length; i++)
			dbPop.populateFromDB(populationDatabase, Y[0]);
...
//		EMPTY_PENALTY_SIGNAL = false; // Check on Rule List
		...
		while(true) {
			...
			else if (currentAct<BodyReturnAction)
