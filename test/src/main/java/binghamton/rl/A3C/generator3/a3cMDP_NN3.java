...
//	StandardPredicate FriendDummyPred; // friend(U,U1)
	...
	public a3cMDP_NN3() {
		maxRuleLen = 5; //6; 5;  
		maxRuleNum = 15; // 8; 12; 15; 18
		
		maxEpochStep = (int)5.0e+5;
		numThread = 2; //2; 3; 
		nstep = 6; // 6, 
		gamma = 1.0; //0.99;
		
		pslModelCreation pslGenerator = new pslModelCreation(123456);
		StandardPredicate[] X = pslGenerator.getX();
		StandardPredicate[] Y = pslGenerator.getY();
		StandardPredicate[] Z = pslGenerator.getZ();
		
		// Dummy Predicates
		StandardPredicate UserDummyPred = (StandardPredicate)PredicateFactory.getFactory().getPredicate("attendsAA");
		StandardPredicate FriendDummyPred = (StandardPredicate)PredicateFactory.getFactory().getPredicate("friends");
		...
		StandardPredicate[] allNegPreds = ArrayUtils.addAll(negFeatPreds, Y); // allPosPreds.clone(); //
...
//			throw new IllegalArgumentException("Max rule body length cannot bigger than Predicates Number");
		...
		actorCriticNN = buildActorCritic(new int[] {inputRow, inputCol}, outputSize);
		asyncGlobal = new AsyncGlobal3(actorCriticNN, maxEpochStep, maxRuleLen, maxRuleNum, gamma);
	}
	...
	public ActorCriticSeparate buildActorCritic(int[] numInputs, int numOutputs) {				
		...
		final int numHiddenNodes = 24; //24; 20; 18; 16; 8
...
//                .regularization(true)
	...
	public AsyncThread3 newThread(int i) {
		return new AsyncThread3(asyncGlobal, i, maxRuleLen, maxRuleNum, nstep, gamma);
	}
...
//				t.run();
	...
	public void saveA3C(String path1, String path2) throws IOException {
		actorCriticNN.save(path1, path2);
	}
