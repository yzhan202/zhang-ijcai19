...
//	StandardPredicate FriendDummyPred; // friend(U,U1)
	...
	public a3cMDP_NN1() {
		maxRuleLen = 4; //6; 5;  
		maxRuleNum = 14; // 8; 12; 15; 18
		
		maxEpochStep = (int)1e+5; // 4.0e+5;
		numThread = 2; //2; 3; 
		nstep = 6; // 6, 
		gamma = 1.0; //0.99;
...
//			throw new IllegalArgumentException("Max rule body length cannot bigger than Predicates Number");
		...
		asyncGlobal = new AsyncGlobal1(actorCriticNN, maxEpochStep, maxRuleLen, maxRuleNum, gamma);
	}
	...
	public ActorCriticSeparate buildActorCritic(int[] numInputs, int numOutputs) {				
		...
		final int numHiddenNodes = 20; //24; 20; 18; 16; 8
...
//                .regularization(true)
	...
	public AsyncThread1 newThread(int i) {
		return new AsyncThread1(asyncGlobal, i, maxRuleLen, maxRuleNum, nstep, gamma);
	}
